...
static void recent_project_activate_cb(GtkMenuItem *menuitem, G_GNUC_UNUSED gpointer user_data)
{
	...
	else if (project_load_from_file_with_session(locale_filename))
...
/**
 * Disables overlay scrolling on window containing provided tree view.
 */
GEANY_API_SYMBOL
void ui_tree_view_disable_overlay_scrollbar(GtkTreeView *tree_view)
{
	g_return_if_fail(GTK_IS_TREE_VIEW(tree_view));
